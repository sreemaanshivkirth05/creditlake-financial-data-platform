...
| Python test suite, portfolio upgrade | 39 passed, zero failures; 50.66 seconds locally | `evidence/pytest.xml` |
| Reproducible portfolio demonstration | Source trace, replay, failed publication, recovery, retained old release, and two cutoff semantics passed | `evidence/portfolio-validation.json` |
...
| Browser interaction QA | Standalone and API-backed views passed; no JavaScript errors | `evidence/dashboard-validation.json` |
| Desktop and mobile rendering | Verified at 1365px and 390px; no page overflow | `../demo/dashboard-*.png` |
...
The portfolio upgrade adds source-to-Parquet lineage verification, named missing
...
The optional Docker/Compose configuration is included but its container build
was not executed because Docker is unavailable in this environment. GitHub
Actions is configured; its equivalent checks passed locally, and no remote CI
execution is claimed. AWS/Snowflake deployment is discussed as an extension,
not included as a performed deployment. Windows and macOS instructions are
provided but those operating systems were not used for validation.

The configured CI now includes Python 3.11/3.12/3.13 jobs and a Docker Compose
serving check. These are configured jobs, not locally verified executions on all
three interpreters or a claimed remote CI result. The standard-library container
smoke script can also target a locally running API independently of Docker.
